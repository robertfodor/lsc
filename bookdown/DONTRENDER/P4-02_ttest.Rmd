...
```{r cogstatloadzeppo, echo=FALSE, fig.cap="The `zeppo.csv` data file loaded into CogStat.", fig.align="center"}
...
```{r cogstatexplorezeppo, echo=FALSE, fig.align="center", fig.cap="The `Explore variable` function in CogStat."}
...
```{r zeppo, fig.cap="The theoretical distribution (dashed green line) from which the psychology student grades (blue bars) are generated in CogStat at the `Population properties` section of our result set.", echo=FALSE, fig.align="center"}
...
```{r ztest1, fig.cap="Rejection regions for the one-sided $z$-test", echo=FALSE, fig.align="center"}
  plot.new()
...
```{r ztest2, fig.cap="Rejection regions for the two-sided $z$-test", echo=FALSE, fig.align="center", fig.show="hold"}
  width <- 6
  height <- 4
...
```{r centraltendencyexplorevar, echo=FALSE, fig.align="center"}
knitr::include_graphics("resources/image/centraltendency.png")
```

```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/paramest1.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/paramest1.txt")
  )
}
```
...
```{r ttestdist, fig.cap="The $t$ distribution with 2 degrees of freedom (left) and 10 degrees of freedom (right), with a standard normal distribution (i.e., mean 0 and std dev 1) plotted as dotted lines for comparison purposes. Notice that the $t$ distribution has heavier tails (higher kurtosis) than the normal distribution; this effect is quite exaggerated when the degrees of freedom are very small, but negligible for larger values. In other words, for large $df$ the $t$ distribution is essentially identical to a normal distribution.", echo=FALSE, fig.align="center"}
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp4.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp4.txt")
  )
}
```
...
```{r harpodata, echo=FALSE, fig.align="center"}
...
```{r harpocompare, echo=FALSE, fig.align="center", fig.show="hold", fig.cap="Comparing grades by tutor in CogStat for our `harpo` data set."}
...
```{r harpoboxplot, fig.align="center", echo=FALSE}
...
```{r cogstatharpohypo, fig.align="center", echo=FALSE}
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/paramest2.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/paramest2.txt")
  )
}
```
...
```{r harpowelch, fig.cap="CogStat result set when the Levene test is significant, hence a Welch test must be run.", echo=FALSE, fig.align="center"}
...
(ref:chicocap) CogStat descriptives and boxplot for the variables `grade_test1` and `grade_test2` in the Compare repeated measures variables function.

```{r chico, fig.cap=paste("(ref:chicocap)"), echo=FALSE, fig.align="center", fig.show="hold"}
...
```{r pairedtb, fig.cap="Scatterplot showing the individual grades for test 1 and test 2", echo=FALSE, fig.align="center"}
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/paramest3.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/paramest3.txt")
  )
}
```

```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp5.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp5.txt")
  )
}
```
...
```{r cohensdinterpretation, echo=FALSE, fig.align="center"}
knitr::kable(tibble::tribble(
          ~V1,                    ~V2,
...
```{r zeppore, fig.cap="Left: a histogram of the data points (observations) in the `zeppo.csv` data set with a green dashed line showing the normal distribution curve. Right: a QQ plot of the same.", echo=FALSE, fig.align="center"}
knitr::include_graphics("resources/image/zeppohist.png")
```
...
```{r aflsmallqq, fig.cap="Left: a histogram of the data points (observations) in the `aflsmall.csv` data set with a green dashed line showing the normal distribution curve. Right: a QQ plot of the same showing how the data points curve and don't form a stright line.", echo=FALSE, fig.align="center"}
...
(ref:swdistcap) Sampling distribution of the Shapiro-Wilk $W$ statistic, under the null hypothesis that the data are normally distributed, for samples of size 10, 20 and 50. Note that *small* values of $W$ indicate departure from normality.

```{r swdist, fig.cap=paste("(ref:swdistcap)"), echo=FALSE, fig.align="center"}
...
### Two-sample Wilcoxon test (Mann-Whitney test){#mannwhitney}
...
Let us recall our AFL example with the data set called [`afl24.csv`](resources/data/afl24.csv). We want to see if there is any statistically significant difference in the `attendance` if the game is a final or not (boolean variable name: `isfinal`).

(ref:mwafl24cap) AFL final and non-final attendance (`Compare groups` function used).

```{r mwafl24, echo=FALSE, fig.align="center", fig.show="hold", fig.cap=paste("(ref:mwafl24cap)")}
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp6.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp6.txt")
  )
}
```
...
### One-sample and paired samples Wilcoxon tests{#wilcoxon}

What about the **one-sample Wilcoxon test**, or equivalently, the **paired samples Wilcoxon test**? Suppose we're interested in finding out whether taking a biology class has any effect on the happiness of students. The data is stored in the [`happiness.csv`](resources/data/happiness.csv) file.
...
```{r mwbeforeafter, echo=FALSE, fig.align="center", fig.show="hold", fig.cap="Happiness before and after statistics class (`Compare repeat measures variables` function used)."}
knitr::include_graphics(c(
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp7.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp7.txt")
  )
}
```
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp8.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp8.txt")
  )
}
```
