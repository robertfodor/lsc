...
- `Testing if the distributions are the same.`: This, in plain English, tells us that we are testing for a null hypothesis where all distributions of all group, or probabilities, are the same. It does not differ in essence from the $H_0$ we described more eloquently in Table \@ref(tab:chapeknullhypo).
...
- `Running chi-squared test.`: Based on all the above, CogStat has decided to run a $\chi^2$ test.
...
CogStat (and many other software, for that matter) introduces this correction, so it's useful to know what it is about. You won't know when it happens because the CogStat output doesn't explicitly say that it has used a "continuity correction" or "Yates' correction".^[Technically, CogStat uses `chi2_contingency` function from `scipy` without specifying the `correction` parameter which defaults to `true`.]

Let us overwrite all the `puppy` answers in our `chapek9` data frame to look at 1 degree of freedom (Figure \@ref(fig:chapek9two)). Let's use the [`chapek9two.csv`](resources/data/chapek9two.csv) data set for this.
