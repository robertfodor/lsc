...
```{r scatterparent1, fig.align='center',echo=FALSE, fig.cap="Scatterplot drawn by CogStat showing the relationship between `parentsleep` and `parentgrump` and between `babysleep` and `parentgrump`", out.width="45%",  fig.show="hold", fig.pos="h"}
...
Scatterplots can only be used with [*continuous variables*](#continuousdiscrete). If you have a [*discrete variable*](#continuousdiscrete), you can use a boxplot instead (see Chapter \@ref(boxplots)).
::::
