...
In past chapters, we compared two groups for the same variable (Chapter \@ref(anova)), or we compared two variables (Chapter \@ref(ttest)). In this chapter, we are going to look at more than one grouping variables, which we sometimes refer to as **factorial ANOVA**.
...
```{r anova2clinicalbox, echo=FALSE, fig.align="center", fig.show="hold"}
knitr::include_graphics("resources/image/cogstatanova2boxplotclinical.png")
```

But sometimes, we want to dissect our data in a different output format. To do that, we have the `Pivot table` function in CogStat. Let's use it to create a cross-tabulation of the means of `mood_gain` for each combination of `drug` and `therapy`.

(ref:anova2clinicalpivotcap) Cross-tabulation of the means of `mood_gain` for each combination of `drug` and `therapy` in `Pivot table` function. You can select different functions to tabulate: *N* (count), *Sum*, *Mean*, *Median*, *Lower* and *Upper quartile*, *Standard deviation*, and *Variance*.

```{r anova2clinicalpivot, echo=FALSE, fig.align="center", fig.show="hold", fig.cap=paste("(ref:anova2clinicalpivotcap)")}
...
knitr::kable(
    ...
    col.names = c("", ""),
    booktabs = TRUE, align = "ll", format = "markdown", escape = FALSE
)
```
...
knitr::kable(
    ...
    col.names = c("", ""),
    booktabs = TRUE, align = "ll",  escape = FALSE, format = "markdown"
)
```
...
knitr::kable(
    ...
    col.names = c("", "No therapy", "CBT", "Total"),
    booktabs = TRUE, align = "lccc",  escape = FALSE, format = "markdown"
)
```
...
```{r maineffects, fig.cap="Factor main effects", echo=FALSE, fig.align="center", out.width="45%", fig.show="hold"}
knitr::include_graphics(c(
...
```{r interaction, fig.cap="Qualitatively different interactions for a $2 \\times 2$ ANOVA", echo=FALSE, fig.align="center", out.width="45%", fig.show="hold"}
knitr::include_graphics(c(
...
```{r interactionplot, fig.cap="Interaction plot for our clinical trial data", echo=FALSE, fig.align="center", fig.show="hold"}
knitr::include_graphics(c(
...
htmltools::includeHTML("resources/html/hyp10.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp10.txt")
  )
}
```
...
```{r echo=FALSE, fig.align="center", fig.cap="Effect sizes for the ANOVA without the interaction term"}
knitr::kable(
    rbind(
        c("Drug", "0.713", "0.789"),
...
```{r cogstatafci, echo=FALSE, fig.align="center", fig.cap="Population parameter estimations: estimated group means -- reduced model"}
...
```{r echo=FALSE, fig.align="center"}
estgroup <- as.data.frame(
  rbind(
        c("Placebo", "No therapy", "0.300", "0.135", "0.006", "0.594"),
        c("Placebo", "CBT", "0.600", "0.135", "0.306", "0.894"),
        c("Anxifree", "No therapy", "0.400", "0.135", "0.106", "0.694"),
        c("Anxifree", "CBT", "1.033", "0.135", "0.740", "1.327"),
        c("Joyzepam", "No therapy", "1.467", "0.135", "1.173", "1.760"),
        c("Joyzepam", "CBT", "1.500", "0.135", "1.206", "1.794")
    )
)

estcap <- "Estimated group means and confidence intervals"
...
library(kableExtra)
    ...
    add_header_above(c(" " = 1, "Mean" = 1,
                "Std. Error" = 1, "95% Confidence Interval" = 2)) %>%
    kableExtra::group_rows(group_label = "Placebo", start_row = 1, end_row = 2) %>%
    kableExtra::group_rows(group_label = "Anxifree", start_row = 3, end_row = 4) %>%
    kableExtra::group_rows(group_label = "Joyzepam", start_row = 5, end_row = 6)
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
    knitr::kable(estgroup,
        col.names = c("Drug", "Therapy", "Mean", "Std. Error",
        "Lower Bound", "Upper Bound"),
        booktabs = TRUE, escape = FALSE,
        caption = estcap,
        align = "llcccc"
        ...
        collapse_rows(columns = 1:1, valign = "middle", latex_hline = "major",
...
knitr::kable(
    rbind(
        c("anxifree-no\\_therapy",  "0.100", "-0.540", "0.740", "0.994"),
        c("joyzepam-no\\_therapy",   "1.167", "0.527", "1.807", "0.001"),
        c("placebo-CBT",  "0.300", "-0.340", "0.940", "0.628"),
        c("anxifree-CBT",  "0.733", "0.093", "1.373", "0.022"),
        c("joyzepam-CBT",  "1.200", "0.560", "1.840", "0.000"),
        c("joyzepam-no\\_therapy",  "1.067", "0.427", "1.707", "0.001"),
    ...
    kableExtra::group_rows("... - placebo-no_therapy", 1, 5) %>%
    kableExtra::group_rows("... - anxifree-no_therapy", 6, 9) %>%
    kableExtra::group_rows("... - joyzepam-no_therapy", 10, 12) %>%
    kableExtra::group_rows("... - placebo-CBT", 13, 14) %>%
    kableExtra::group_rows("... - anxifree-CBT", 15, 15)
```
...
```{r, echo=FALSE, fig.align="center", fig.cap="The coffee data set"}
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp11.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp11.txt")
  )
}
```
...
```{r, eval=knitr::is_html_output(), echo=FALSE}
htmltools::includeHTML("resources/html/hyp12.html")
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output(
      xfun::file_string("resources/html/hyp12.txt")
  )
}
```
...
knitr::kable(tibble::tribble(
  ...
  ), col.names = c("", ""), escape = FALSE, booktabs = TRUE, format="markdown")
```
...
knitr::kable(
    ...
    booktabs = TRUE,
    col.names = c("Source", "Type II SS", "df", "MS", "$F$", "$p$"),
    align = "l", escape = FALSE
) %>%
    kable_styling(position = "center", full_width = FALSE, latex_options = "hold_position")
```
