...
knitr::include_graphics("resources/srs1.png")
```
...
knitr::include_graphics("resources/brs.PNG")
```
...
knitr::include_graphics("resources/srs2.png")
```
...
knitr::include_graphics("resources/IQdista.svg")
```
...
```{r IQdistb, fig.cap="A sample of 100 observations drawn from the population distribution of IQ scores.", echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("resources/IQdistb.svg")
```
...
```{r IQdistc, fig.cap="A sample of 10,000 observations drawn from the population distribution of IQ scores.", echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("resources/IQdistc.svg")
```
...
knitr::include_graphics("resources/sampleDist4.png")
```
...
knitr::include_graphics("resources/sampleDistMax.png")
```
...
knitr::include_graphics(c("resources/cltDemo-a.svg",
						"resources/cltDemo-b.svg",
						"resources/cltDemo-c.svg",
						"resources/cltDemo-d.svg"))
```
...
knitr::include_graphics("resources/samplingDistSampleSD.png")
```
...
```{r estimatorbias, fig.cap="An illustration of the fact that the sample mean is an unbiased estimator of the population mean (panel a). Still, the sample standard deviation is a biased estimator of the population standard deviation (panel b). To generate the figure, we have 10,000 simulated data sets with 1 observation each, 10,000 more with 2 observations, and so on up to a sample size of 10. Each data set consisted of fake IQ data: the data were normally distributed with a true population mean of 100 and standard deviation 15. *On average*, the sample means turn out to be 100, regardless of sample size (panel a). However, the sample standard deviations turn out to be systematically too small (panel b), especially for small sample sizes.", echo=FALSE, fig.show="hold", fig.align="center", out.width="50%"}
knitr::include_graphics(c("resources/biasMean.png",
                          "resources/biasSD.png"))
```

The fix to this systematic bias turns out to be very simple. Here's how it works. Before tackling the standard deviation, let's look at the variance. If you recall from Section \@ref(var), the sample variance is defined as the average squared deviations from the sample mean. That is:
$$
...
The formula above for the 95\% confidence interval is approximately correct, but we glossed over an important detail in the discussion. Notice that it requires you to use the standard error of the mean, SEM, which in turn requires you to use the true population standard deviation $\sigma$. Yet, in Section \@ref(pointestimates), we stressed the fact that we don't actually *know* the true population parameters. Because we don't know the true value of $\sigma$, we have to use an estimate of the population standard deviation $\hat{\sigma}$ instead. This is pretty straightforward to do, but this has the consequence that we need to use the quantiles of the $t$-distribution rather than the normal distribution to calculate our magic number. And the answer depends on the sample size.
...
knitr::include_graphics(c("resources/confIntReplicatedA.png",
                          "resources/confIntReplicatedB.png"))
```
...
So, for example, let us load the `afl24.Rdata` file again and explore the `attendance` variable with the `Explore variable` tool.

```{r, echo=FALSE, fig.cap="Sample properties for `attendance`", fig.align='center', out.width='100%'}
knitr::include_graphics("resources/Cogstatattendancesample.png")
```
...
```{r, echo=FALSE, fig.cap="Population parameter estimates for `attendance`", fig.align='center', out.width='100%'}
knitr::include_graphics("resources/Cogstatattendancepopulation.png")
```
