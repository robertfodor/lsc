...
```{r cogcompbydrugdialog, echo=FALSE, fig.align="center", out.width="100%", fig.show="hold", fig.cap="CogStat dialogue for `Compare groups` function with our clinical trial data set with variables `mood_gain` grouped by `drug`."}
knitr::include_graphics(c(
    "resources/image/comparegroupsbydrug.png"))
```

```{r cogcompbydrugdescr, echo=FALSE, fig.align="center", out.width="100%", fig.show="hold", fig.cap="CogStat result set displaying descriptives and boxplots as a result of `Compare groups` function with our clinical trial data set with variables `mood_gain` grouped by `drug`."}
knitr::include_graphics(c(
    "resources/image/moodbydrug.png"))
```

```{r cogcompbydrug, echo=FALSE, fig.align="center", out.width="100%", fig.show="hold", fig.cap="CogStat result set with hypothesis test results, effect size and post hoc results after running `Compare groups` function with our clinical trial data set with variables `mood_gain` grouped by `drug`."}
knitr::include_graphics(c(
...
In our case, we see in the first result line that Joyzepam is significantly better than Anxifree, because the mean difference is positive (meaning an improvement from Anxifree towards Joyzepam), our $p$-value is less than 0.05, and the confidence interval does not include zero. The last column indicates we rejected the null hypothesis.

We also see in the third line that Joyzepam is significantly better than the placebo, because the mean difference is negative from Joyzepam towards placebo. The $p$-value is less than 0.05, and the confidence interval does not include zero, and the last column indicates we rejected the null hypothesis.

The second line, from Anxifree towards the placebo, we see a negative mean difference, meaning that while Anxifree looks like it's performing better than the placebo (as we've seen in the boxplot as well), the confidence interval does include zero, and the $p$-value is greater than 0.05, so we fail to reject the null hypothesis.
...
```{r echo=FALSE, fig.cap="Frequency table for the raw data (`mood_gain`).", fig.align="center", out.width="100%"}
knitr::include_graphics("resources/image/cogstatmoodgainfrequency.png")
```

Looking at this table, notice that the the value of $0.3$ (and $0.6$, $1.3$, $1.4$) has a frequency of 2. This is telling us that two people's mood increased by 0.3 (and 0.6...). More to the point, note that we can say $f_3 = 2$, where 3 is the index of the value (i.e. row number 3). Yay. So, now that we know this, the tie correction factor (TCF) is:
$$
